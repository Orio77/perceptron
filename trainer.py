...
class Trainer:
  ...
  def train(self, x_train: np.ndarray, y_train: np.ndarray, x_test: np.ndarray, y_test: np.ndarray):
    ...
    for epoch in range (self.n_epochs):
        A = self.perceptron.forward(X=x_train)

        L = self.loss(A=A, y=y_train)
        self.loss_track.append(L)

        dw, db = self.backward(X=x_train, A=A, y=y_train)

        self.update_weights(dw=dw, db=db)

        y_pred = self.predict(X=x_test)

        accuracy = self.accuracy(y_pred=y_pred, y_true=y_test)

        if epoch % 100 == 0:
          self.print_stats(epoch=epoch, L=L, acc=accuracy)
...
if __name__ == "__main__":
  ...
  final_loss = trainer_train.loss_track[-1]
  print(f"Final loss: {final_loss:.4f}")
  
  assert final_loss < initial_loss, "Training should decrease loss"
  assert len(trainer_train.loss_track) == 100, "Loss track length should equal n_epochs"
